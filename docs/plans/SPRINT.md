...
## 第 6 轮 — 产品方向讨论（AririgiAgent，2026-05-12）

> **来源邮件**：uid:12 "CodeMemory 产品方向 — 从单机工具到协作平台"
> **性质**：产品策略讨论，非功能需求。不生成实施任务。

### 已处理邮件 UID（续）

| UID | 日期 | 主题 | 轮次 |
|-----|------|------|------|
| 11 | 2026-05-12 17:15 | R4 审查闭环确认 | — （确认） |
| 12 | 2026-05-12 17:25 | CodeMemory 产品方向 | R6（讨论） |

### 6 个产品方向评估

| # | 方向 | 契合度 | 评估 |
|---|------|--------|------|
| 1 | 记忆协作层 | 中 | 方向对（"外部大脑"定位天然多 agent），但过早。目前单用户场景还未穷尽，协作层引入的冲突裁决、author 追踪、merge 逻辑是数量级复杂度提升。先做好单用户完整体验。 |
| 2 | 记忆健康度面板 | **高** | 最契合当前阶段。validate + diff + orphans 已有数据管道，前端 Dashboard 已有框架。本质是"被动消费"改造——从主动跑命令变为被动查看。可拆分为小迭代。 |
| 3 | 记忆分支 | 低 | TransientDAG 已覆盖"实验不污染主线"场景。持久化分支引入 git-branch 级别的复杂度（merge 冲突、rebase），ROI 不高。Git 本身就是分支工具，记忆分支可能过度抽象。 |
| 4 | 记忆时间旅行 | 中高 | 技术上已有基础——diff 快照 + change_log。`codememory at "3 days ago"` 的实现路径清晰：从 snapshot 恢复索引 + 对应 .md 文件的历史版本。与 diff 形成互补（diff 告诉你变了什么，at 让你回到过去）。 |
| 5 | 记忆模板市场 | 低 | 冷启动问题——没有社区就没有市场，没有市场就没有社区。当前更务实的做法是继续沉淀 examples/ 下的示例数据集（companion/investment/software-architecture/quant_operators），它们就是最好的"模板"。 |
| 6 | 记忆衰减可视频 | **高** | 与当前架构深度契合：lifecycle + access_count + stability + days_since_last_access 字段全部就绪。Dashboard 已有衰减风险面板的雏形。余下的是前端视觉设计工作——热力条、颜色渐变、"降低温"警告线。 |

### 优先级建议

```
高优先级（可近期启动）:
  2. 记忆健康度面板 — 数据管道就绪，前端框架已有
  6. 衰减可视化反馈 — 数据字段就绪，UI 层工作

中优先级（需要前置条件）:
  4. 记忆时间旅行 — 依赖 snapshot 基础设施（R4 已就绪）

低优先级（哲学冲突或过早）:
  1. 记忆协作层 — 方向对但时机过早
  3. 记忆分支 — TransientDAG 已覆盖核心场景
  5. 记忆模板市场 — 社区冷启动问题
```

### 与项目哲学的对照

CodeMemory 的核心理念是"记忆加载是依赖解析问题，不是搜索问题"。
产品化应该加强这个定位而非稀释它：

- 健康度面板和衰减可视化**加强**定位——让 DAG 的健康状态可感知
- 协作层和时间旅行**扩展**定位——从单人单次到多人多次
- 分支和市场**可能稀释**定位——引入与核心理念无关的复杂度


## 第 7 轮 — 产品方向讨论回应（AririgiAgent，2026-05-12）

> **来源邮件**：uid:13 "产品方向讨论回应 — 加强 vs 稀释的筛子"
> **性质**：讨论回应 + 1 个文档任务
...
| UID | 日期 | 主题 | 轮次 |
|-----|------|------|------|
| 13 | 2026-05-12 17:35 | 产品方向讨论回应 | R7 |

### 讨论要点

- 健康面板 + 衰减可视：共识为最佳下一步
- 分支被拒：同意——记忆 DAG 天然有向无环，不存在"合并冲突"，git 分支模式不适配
- 模板市场：同意——等 examples 自己长成有说服力的模板
- 协作层时机：同意——先把单人场景做到极致

### 任务 1："加强 vs 稀释" 设计哲学写入 CLAUDE.md

| # | 子任务 | 说明 | 状态 |
|---|--------|------|------|
| 1.1 | CLAUDE.md 新增设计哲学节 | 记录核心判断标准：每个新功能应加强"记忆 = DAG 依赖解析"定位，而非稀释。作为内部决策 + 外部贡献的边界锚点。 | [x] |
